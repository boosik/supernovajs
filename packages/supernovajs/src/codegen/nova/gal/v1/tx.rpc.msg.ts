...
export class MsgClientImpl implements Msg {
  ...
  constructor(rpc: Rpc) {
    ...
    this.pendingUndelegate = this.pendingUndelegate.bind(this);
  ...
  pendingUndelegate(request: MsgPendingUndelegate): Promise<MsgPendingUndelegateResponse> {
    const data = MsgPendingUndelegate.encode(request).finish();
    const promise = this.rpc.request("nova.gal.v1.Msg", "PendingUndelegate", data);
    return promise.then(data => MsgPendingUndelegateResponse.decode(new _m0.Reader(data)));
  }
